#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read-only start-pose and semantic-mapping checker for the 5100 policy.

The tool never commands a motor. It compares the current 12 motor angles with
ONNX model default joint angles after applying the repository's semantic
mapping. Candidate zero offsets are printed only when the operator explicitly
states that the robot has been physically placed at the model default pose.
"""
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="5100 ONNX path containing fanfan_deployment_config metadata",
    )
    parser.add_argument(
        "--motor-url",
        default="http://127.0.0.1:8000",
        help="FastAPI motor server base URL",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Print candidate zero offsets. Use only after the supported robot is "
            "manually placed at the exact ONNX default pose. No file is modified."
        ),
    )
    return parser
...
def main(argv=None) -> int:
    ...
    if not snap.valid:
        raise RuntimeError("motor snapshot is invalid or server state cache is stale")
    ...
    if args.assume_physically_at_model_default:
        ...
        print("\nCandidate real_zero_offset_policy_order (NOT applied automatically):")
        print("[" + ", ".join(f"{float(x):+.6f}" for x in candidate) + "]")
        print(
            "Only use these values after independently verifying every joint sign "
            "and that the supported robot was exactly at the model default pose."
        )
