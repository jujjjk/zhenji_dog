...
# Final URDF version used for this hardware bring-up:
# fanfan_mass_scaled_only_trunk_plus_800g.urdf
# - total mass: about 7.242 kg
# - trunk/belly mass: about 2.762 kg
# - each leg mass: about 1.120 kg
#
# The gait below is intentionally NOT an RL policy. It is a conservative
# real-order open-loop gait for checking whether the 3D-printed soft body can
# carry weight and whether each leg can lift without dragging.
#
# Real motor order used by the existing controller:
# FR: 0x11 hip, 0x12 thigh, 0x13 calf
# FL: 0x21 hip, 0x22 thigh, 0x23 calf
# RL: 0x31 hip, 0x32 thigh, 0x33 calf
# RR: 0x41 hip, 0x42 thigh, 0x43 calf


DEFAULT_STAND_POSE_REAL_ORDER = [
    # Narrower than the previous +/-0.157 hip pose. This reduces the visual
    # "legs splayed out" posture on a soft 3D-printed body.
    (0x11, 0.100000),   # FR_hip
    (0x12, 0.360000),   # FR_thigh
    (0x13, -0.820000),  # FR_calf

    (0x21, -0.100000),  # FL_hip
    (0x22, -0.360000),  # FL_thigh
    (0x23, 0.820000),   # FL_calf

    # Rear legs are slightly more bent than the old 0.349 rad calf pose,
    # giving more swing clearance margin before the foot drags.
    (0x31, 0.100000),   # RL_hip
    (0x32, -0.260000),  # RL_thigh
    (0x33, 0.580000),   # RL_calf

    (0x41, -0.100000),  # RR_hip
    (0x42, 0.260000),   # RR_thigh
    (0x43, -0.580000),  # RR_calf
]


REAL_LEG_TABLE = {
    # index order in target_real, hip sign, thigh sign, calf-bending sign
    # calf-bending sign means positive value makes the knee more bent.
    "FR": {"idx": (0, 1, 2), "hip_sign": +1.0, "thigh_sign": +1.0, "calf_sign": -1.0},
    "FL": {"idx": (3, 4, 5), "hip_sign": -1.0, "thigh_sign": -1.0, "calf_sign": +1.0},
    "RL": {"idx": (6, 7, 8), "hip_sign": +1.0, "thigh_sign": -1.0, "calf_sign": +1.0},
    "RR": {"idx": (9, 10, 11), "hip_sign": -1.0, "thigh_sign": +1.0, "calf_sign": -1.0},
}
...
class MydogOpenLoopGaitNode(Node):
    ...
    def __init__(self):
        super().__init__("mydog_soft_body_gait_node")
        ...
        self.declare_parameter("step_hz", 0.35)
        self.declare_parameter("motion_mode", "soft_crawl")
        ...
        self.declare_parameter("send_kp", 35.0)
        ...
        self.declare_parameter("rate_limit_rad_per_sec", 0.80)

        # Soft-body real-order gait parameters. These are intentionally small.
        # Increase only after checking motor current/temperature and foot clearance.
        self.declare_parameter("soft_stride_thigh_amp", 0.055)
        self.declare_parameter("soft_swing_calf_amp", 0.260)
        self.declare_parameter("soft_rear_extra_calf_amp", 0.060)
        self.declare_parameter("soft_stance_calf_amp", 0.015)
        self.declare_parameter("soft_hip_balance_amp", 0.000)
        self.declare_parameter("soft_lift_only", False)
        ...
        self.rate_limit_rad_per_sec = float(self.get_parameter("rate_limit_rad_per_sec").value)
        self.soft_stride_thigh_amp = float(self.get_parameter("soft_stride_thigh_amp").value)
        self.soft_swing_calf_amp = float(self.get_parameter("soft_swing_calf_amp").value)
        self.soft_rear_extra_calf_amp = float(self.get_parameter("soft_rear_extra_calf_amp").value)
        self.soft_stance_calf_amp = float(self.get_parameter("soft_stance_calf_amp").value)
        self.soft_hip_balance_amp = float(self.get_parameter("soft_hip_balance_amp").value)
        self.soft_lift_only = bool(self.get_parameter("soft_lift_only").value)
        ...
        self.get_logger().info(
            ...
            f"stride_sign={self.stride_sign:+.1f}, rate_limit={self.rate_limit_rad_per_sec:.2f}rad/s, enable_send={self.enable_send}, "
    ...
    def update(self):
        ...
        if elapsed < self.stand_sec:
            ...
            if self.motion_mode in ("soft_crawl", "soft_walk", "soft_trot", "leg_lift_check", "toe_tap"):
                # Build directly in REAL motor order. This avoids relying on policy
                # semantic mapping when we only want a safe hardware visual test.
                target_policy = self.default_policy.copy()
                target_real = self.build_soft_real_gait_target(phase_value, warm)
            else:
        ...
        # Extra rate limiter for soft 3D-printed hardware. The existing max_delta
        # check only skips large jumps; this limiter actively smooths every command.
        target_real = self.apply_rate_limit(target_real, dt)
    ...
    def apply_rate_limit(self, target_real: np.ndarray, dt: float) -> np.ndarray:
        target_real = np.asarray(target_real, dtype=np.float32).reshape(12)
        max_step = max(0.0, self.rate_limit_rad_per_sec) * max(float(dt), 1e-3)
        if max_step <= 0.0:
            return target_real
        delta = np.clip(target_real - self.last_target_real, -max_step, max_step)
        return (self.last_target_real + delta).astype(np.float32)

    def build_soft_real_gait_target(self, phase: float, warm: float) -> np.ndarray:
        """
        Conservative real-order gait for the 7.24 kg soft-body URDF.

        Modes:
        - soft_crawl / soft_walk: one leg swings at a time, high support duty.
        - leg_lift_check / toe_tap: one leg lifts at a time with almost no stride.
        - soft_trot: diagonal preview only; use while the robot is supported.
        """
        q = self.default_real.copy()

        if self.motion_mode in ("leg_lift_check", "toe_tap"):
            offsets = {"FR": 0.00, "RL": 0.75, "FL": 0.50, "RR": 0.25}
            duty = 0.86
            stride_amp = 0.0
            calf_amp = self.soft_swing_calf_amp * 0.85
        elif self.motion_mode == "soft_trot":
            offsets = {"FR": 0.00, "RL": 0.00, "FL": 0.50, "RR": 0.50}
            duty = max(0.62, min(self.duty_factor, 0.75))
            stride_amp = self.soft_stride_thigh_amp * 0.75
            calf_amp = self.soft_swing_calf_amp * 0.75
        else:
            # Crawl sequence: FR -> RL -> FL -> RR. At least three legs stay
            # near support most of the time, which is safer for the printed body.
            offsets = {"FR": 0.00, "RL": 0.75, "FL": 0.50, "RR": 0.25}
            duty = max(0.76, min(self.duty_factor, 0.90))
            stride_amp = self.soft_stride_thigh_amp
            calf_amp = self.soft_swing_calf_amp

        swing_fraction = max(0.06, 1.0 - duty)

        for leg, offset in offsets.items():
            spec = REAL_LEG_TABLE[leg]
            hip_i, thigh_i, calf_i = spec["idx"]
            p = (phase + offset) % 1.0
            ...
            if p < swing_fraction:
                ...
                s = (p - swing_fraction) / max(duty, 1e-3)
                lift = 0.0
                # Slow return during stance. This is small because open loop has
                # no body velocity feedback.
                stride = 1.0 - 2.0 * s
                support = math.sin(math.pi * s)

            # Rear legs need a little more calf bend because the old real data
            # often showed rear toes dragging rather than lifting.
            rear_extra = self.soft_rear_extra_calf_amp if leg in ("RL", "RR") else 0.0
            leg_calf_amp = calf_amp + rear_extra

            if not self.soft_lift_only:
                q[thigh_i] += warm * spec["thigh_sign"] * stride_amp * stride
                q[hip_i] += warm * spec["hip_sign"] * self.soft_hip_balance_amp * stride

            # Positive "leg_calf_amp" means "bend this leg more"; calf_sign handles
            # the left/right/rear sign differences in real motor order.
            q[calf_i] += warm * spec["calf_sign"] * (
                leg_calf_amp * lift
                + self.soft_stance_calf_amp * support
            )
