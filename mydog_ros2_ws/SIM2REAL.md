...
读取 ONNX 中的12维分关节增益。真机保持 `model_kp_scale=1.0`，并针对上一轮
欠阻尼抖动使用 `model_kd_scale=2.0`，因此有效增益为髋关节 `60/1.2`、
大腿和小腿 `70/1.6`。只有显式设置 `use_model_pd_gains:=false` 时才使用标量回退值。
...
策略只训练了前进速度约 `0.15-0.30 m/s`，测试命令从中间值开始：

看到 `READY` 后再持续发布命令（停止发布超过 0.5 秒后策略不再发送电机目标）：

```bash
ros2 topic pub -r 10 /cmd_vel geometry_msgs/msg/Twist \
  "{linear: {x: 0.225, y: 0.0, z: 0.0}, angular: {x: 0.0, y: 0.0, z: 0.0}}"
```
...
`/cmd_vel.angular.z` 在部署端被积分成期望航向，再按训练时的 heading controller 生成 yaw 命令和 2 维航向误差观测。
